...
На практике токены генерируются и размещаются в Vault разработчиками вручную, а не выпускаются автоматически при каждом запросе. Чтобы избежать повторной ручной генерации, срок действия таких токенов устанавливается в 10 лет. Это устраняет операционные издержки, однако существенно осложняет ротацию ключей: замена ключа требует инвалидации всех ранее выпущенных долгоживущих токенов.
