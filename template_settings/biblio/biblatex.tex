...
%%% Нормоконтроль: несколько источников в одной ссылке разделяются запятой, а не «;»
%%% (например, [1, 2, 3] вместо [1; 2; 3]). Также сортировка номеров уже включена (sortcites).
\AtBeginDocument{\renewcommand*{\multicitedelim}{\addcomma\space}}

%%% Нормоконтроль: для иноязычных источников дату обращения печатаем по-русски
%%% («дата обращения» вместо «visited on»).
\DefineBibliographyStrings{english}{urlseen = {дата обращения}}