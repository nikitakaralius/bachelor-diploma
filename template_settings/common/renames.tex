...
%%
%%
\newcommand{\keywordsRu}{бесшовная ротация ключей, межсервисная авторизация, JWT-токены, высоконагруженные микросервисные системы, Kubernetes-оператор} % ВВЕДИТЕ ключевые слова по-русски
%%
%%
\newcommand{\keywordsEn}{zero-downtime key rotation, inter-service authorization, JWT tokens, high-load microservice systems, Kubernetes operator} % ВВЕДИТЕ ключевые слова по-английски
%%
%%
...
\newcommand{\abstractRu}{Объект исследования~--- система межсервисной авторизации высоконагруженной микросервисной платформы Mindbox (около 160~микросервисов, свыше 2~млн запросов в минуту). Цель работы~--- спроектировать, реализовать и внедрить систему автоматической бесшовной ротации криптографических ключей подписи JWT. Методология включает моделирование угроз по STRIDE с количественной оценкой DREAD, анализ открытых стандартов (JWT, JWK, OAuth 2.0 Token Exchange, OpenID Connect) и нагрузочное тестирование. Разработана система из пяти компонентов: микросервиса авторизации auth-jwt, Kubernetes-оператора доставки токенов, Terraform-провайдера и модуля конфигурации, клиентской библиотеки. Бесшовность обеспечивает трёхшаговый алгоритм ротации с grace-периодами и проверкой доступности ключей; межконтурное взаимодействие построено на Token Exchange. Использованы C\#, Go, Docker, Kubernetes, PostgreSQL и Yandex Cloud. Система внедрена в промышленную эксплуатацию: ключи ротируются автоматически по расписанию, время ответа по 95-му процентилю~--- 8~мс при SLO 30~мс, подтверждённая пропускная способность~--- 15~000 запросов в секунду, за два месяца эксплуатации инцидентов авторизации не зафиксировано. Результаты применимы в высоконагруженных микросервисных платформах, требующих плановой смены ключей без простоя. Подтверждена гипотеза: бесшовная автоматическая ротация достижима собственным решением на основе открытых стандартов с учётом особенностей существующей архитектуры.} % ВВЕДИТЕ текст аннотации по-русски
%%
%%
\newcommand{\abstractEn}{The object of study is the inter-service authorization system of the high-load microservice platform Mindbox (about 160~microservices, over 2~million requests per minute). The aim is to design, implement and deploy a system for automated zero-downtime rotation of cryptographic JWT signing keys. The methodology comprises STRIDE threat modeling with quantitative DREAD scoring, analysis of open standards (JWT, JWK, OAuth 2.0 Token Exchange, OpenID Connect) and load testing. A system of five components was developed: the auth-jwt authorization microservice, a Kubernetes operator for token delivery, a Terraform provider with a configuration module, and a client library. Zero downtime is ensured by a three-step rotation algorithm with grace periods and key-availability checks; cross-contour interaction relies on Token Exchange. The technologies used are C\#, Go, Docker, Kubernetes, PostgreSQL and Yandex Cloud. The system runs in production: keys are rotated automatically on schedule, the 95th-percentile response time is 8~ms against a 30~ms SLO, the confirmed throughput is 15,000 requests per second, and no authorization incidents occurred over two months of operation. The results apply to high-load microservice platforms that require scheduled key rotation without downtime. The hypothesis is confirmed: zero-downtime automated rotation is achievable with an in-house solution built on open standards and tailored to the existing architecture.} % ВВЕДИТЕ текст аннотации по-английски
