...
% TODO: пирамида тестов + специфика инфраструктурного кода.
% TODO: сводная таблица покрытия по компонентам
%       (компонент × юнит × интеграц. × E2E × нагрузка × хаос).
% TODO: обоснование смещения в сторону интеграционных тестов,
%       найти научный источник (Vassallo 2017 или industry survey).
...
% TODO: юнит тесты тоже есть. Нужно обоснование почему их меньше, чем интеграционных. Найти научное исследование, где доказывается, что интеграционные тесты лучше отлавливают ошибки
...
% TODO: разобрать репозиторий terraform-provider-authjwt:
%       что покрывают юнит-тесты (схема, маппинг, валидация).
% TODO: обоснование, почему acceptance-тесты избыточны
%       (поведение взаимодействия с auth-jwt покрыто E2E оператора
...
% TODO: цель — оценить запас по SLO 30 мс p95.
% TODO: инструмент — Grafana k6, схема стенда.
% TODO: результаты — 15k RPS (потолок инструмента), p95 = 80 мс в пике.
% TODO: сопоставление с production: типовая нагрузка контура Gamma
%       200 RPS, p95 = 9 мс — SLO выдерживается с запасом > 3x.
% TODO: графики latency/RPS/CPU (плейсхолдеры).

\section{Хаос-инжиниринг в промышленной среде}\label{sec:ch4-chaos}

% TODO: Chaos Mesh — ежедневная инъекция сетевого partition
%       в один pod auth-jwt.
% TODO: что проверяется — устойчивость клиентов (retry, кеш токенов
%       в библиотеке), отсутствие cascading failures.
% TODO: критерий успеха — отсутствие алертов и роста error rate.

\section{Апробация в промышленной среде}\label{sec:ch4-rollout}

% TODO: дата внедрения — 2026-03 (≈ 2 месяца назад).
% TODO: текущий статус: ротация выполняется автоматически
%       на всех контурах (alpha, beta, stable).
% TODO: дашборд «время последней ротации» — скриншот.
% TODO: подтверждение гипотезы из введения.

\section{Выводы по главе}\label{sec:ch4-conclusion}

% TODO: сводная таблица «требование → метрика → факт».
% TODO: подтверждение SLO 30 мс p95.
% TODO: подтверждение гипотезы из введения.
